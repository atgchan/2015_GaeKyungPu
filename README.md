...
* 기본 조작법
    ...
    + 병사 선택 - 병사를 클릭
    + 병사 회전 - 병사 선택 후 가운데 클릭(반시계), 우클릭(시계)
    + 병사 이동 - 병사 선택 후 가고자 하는 방향으로 회전, 주변 타일 클릭(forest로의 이동은 식량 2 소모)
    + 병사 공격 - 병사 선택 후 정면 타일(적 병사가 올라가있는) 클릭

* 공식룰 적용
    + 이제 병사가 한 턴에 한 번밖에 이동할 수 없습니다.
    + 이제 병사가 한 번 공격을 하고 난 후에는 이동하거나 공격할 수 없습니다.

* 조작 개선
    + 병사 회전 조작법이 마우스 우클릭, 가운데 클릭으로 바뀌었습니다.

* UI 업데이트
    + 병사 회전 표시가 예쁘고 직관적으로 바뀌었습니다.
    + 공격력 표시 숫자가 눈에 잘 띄게 변경되었습니다.
    + 음소거 버튼 그래픽이 예뻐졌습니다.
    + 게임 종료 화면에서 승패를 확인할 수 있습니다.

* 그래픽 업데이트
    + 옥토 타일에 수염이 달렸습니다.

* 사운드 업데이트
    + 병사 이동 효과음이 띠용 에서 저벅저벅으로 바뀌었습니다.
    + 게임 내 배경음악이 바뀌었습니다.
...
![image](http://ref.comgal.info/data/cgref_4/%EC%B5%9C%EC%8B%A0%EB%B2%84%EC%A0%84%EC%A3%BC%EC%82%AC%EC%9C%84.png)
...
* UI 업데이트
    + 병사 회전 버튼이 더 크게, 직관적으로 변경되었습니다.
    + 공격 시 병사의 공격 데미지(주사위 굴림 결과)가 시각적으로 표시됩니다.
    + 병사의 공격력이 구슬 이미지에서 더 아름다운 글씨로 변경되었습니다.
    + 이제 어떤 플레이어의 턴인지 좌측 상단에 표시됩니다.
    + 이제 현재 턴인 플레이어의 식량만 표시됩니다.
    + 이제 뮤트 버튼이 스피커 모양으로 바뀌었습니다.

* 그래픽 업데이트
    + 병사 그래픽이 완전히 아름답게 변경되었습니다. 병사의 방향과 진형을 파악하기가 쉬워집니다.
    + 인게임 배경화면이 자연스럽게 바뀌었습니다.
    + 배럭 그래픽이 더 자연스러워졌습니다.
    + 시작화면이 위엄있고 멋진 배경으로 바뀌었습니다.
    + 공격대상 병사 머리위의 칼모양이 더 레트로 느낌으로 바뀌었습니다.(절대 마인크래프트에서 베낀게 아닙..)
    + 용암과 물 모양이 더 찐~해졌습니다.

* 사운드 업데이트
    + 인게임 배경음악이 변경되었습니다. 이제 함성이 사라졌습니다..

* 애니메이션 업데이트
    + 공격할 때 상대위치에 맞게 애니메이션이 표시됩니다.
    + 이제 타일이 바뀔 때 더 자연스럽게 바뀝니다.
